sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.zmmtransport.controller.Main", {
		...
		_onSearchFieldLiveChange: function(oEvent) {
			var sControlId =
				"sap_m_Page_0-content-sap_m_IconTabBar-1506348176785-items-sap_m_IconTabFilter-1-content-build_simple_Table-1506356963393";
			...
			return new Promise(function(fnResolve) {
				...
				if (sQuery && sQuery.length > 0) {
					...
					if (!isNaN(iQuery)) {
						aFilters.push(new sap.ui.model.Filter("SHIP_NO", sap.ui.model.FilterOperator.EQ, sQuery));
					}
			...
			if (!oDialog) {
				this.getOwnerComponent().runAsOwner(function() {
					oView = sap.ui.xmlview({
						viewName: "com.sap.build.standard.zmmtransport.view." + sDialogName
					});
		...
		_onTableItemPress1: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				this.doNavigate("View_1", oBindingContext, fnResolve, "");
			}.bind(this)).catch(function(err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
